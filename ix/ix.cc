...
RC IX_IndexHandle::FindEntryPage(const void *key, uint16_t &pageNum, const bool doSplit) const
{
  ...
  int search_key_size = getKeySize(key);
  ...
  while (type == IX_NODE)
	...
	{  
	  ...
	  int cmp = IX_Manager::keycmp(key, ix_key, search_key_size, key_size, shift_offset);
	  if (cmp == 0)
	    {
